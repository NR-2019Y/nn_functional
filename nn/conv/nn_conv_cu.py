...
def conv2d_nhwc_v1(img: cp.ndarray, kernel: cp.ndarray, stride: size_2_t = 1, dilation: size_2_t = 1) -> cp.ndarray:
...
def conv2d_nchw_v1(img: cp.ndarray, kernel: cp.ndarray, stride: size_2_t = 1, dilation: size_2_t = 1) -> cp.ndarray:
...
def conv2d_nchw_v2(img: cp.ndarray, kernel: cp.ndarray, stride: size_2_t = 1, dilation: size_2_t = 1) -> cp.ndarray:
    batch_size, _ic, ih, iw = img.shape
    oc, ic, kh, kw = kernel.shape
    assert _ic == ic
    sh, sw = get_pair(stride)
...
def _check():
    ...
    def conv2d_torch(img: cp.ndarray, kernel: cp.ndarray, stride: size_2_t = 1, dilation: size_2_t = 1) -> cp.ndarray:
        x = from_dlpack(toDlpack(img))
        w = from_dlpack(toDlpack(kernel))
        # tic = time.time()
        r = F.conv2d(x, w, bias=None, stride=stride, dilation=dilation)
        # print(f'F.conv2d {(time.time() - tic) * 1000:.4f} ms')
        return cupy.from_dlpack(to_dlpack(r))

    x = cp.random.rand(8, 128, 200, 210)  # .astype(cp.float32)
    k = cp.random.rand(1024, 128, 3, 3)  # .astype(cp.float32)
